	...
	export default {
		...
		methods:{
			async identificationMy() {
				...
				this.fileList = res.data.job.map(item => ({url: item.url}));
				this.status = res.data.job_certification_status;
			},
			async onSubmit() {
				...
				const res = await this.$apis.uesrApi.identificationWork(this.fileList.map(item => item.url));
				if (res.status) {
