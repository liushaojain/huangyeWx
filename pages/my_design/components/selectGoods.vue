<template>
	<view class="choose">
		...
		<view class="allGoods">
			<view class="goods" v-if="partsAll.bag.length>0">
			<!-- <view class="goods" v-if="partsAll.bag.length>0&&addInfo.eidtWinDiy!='true'"> -->
				<view class="goodsType">选购手提袋</view>
				<view class="goodsContainer">
					<view class="goodsItem" :class="{'checked':item.checked}" @tap="checkparts(index,partsAll.bag,'bag')" v-for="(item,index) in partsAll.bag" :key="index">
		...
		<view class="groupBtn">
			...
			<button class="comfirmGoods" @tap="next">确认新增商品</button>
		</view>
	</view>
</template>
...
<script>
	...
	export default {
		...
		props:["activeGoodsId","activeProductId","packNum","addInfo"],
		created() {
		...
		methods:{
			...
			recommendGoods(changeBuy) {
				...
				this.$request(productApi.recommendGoods, {
					...
					let relevanceId = [];
					let bottleLableId = this.addInfo.bottle_lable;
					...
					if(this.addInfo.bag==false&&this.addInfo.eidtWinDiy=="false"){
						for(let bag of data.bag){
							if(bag.winDiy=="false"){
								this.partsAll.bag.push(bag)								
							}
						}
					}
					if(relevanceId&&relevanceId.length&&this.addInfo.bottle_cover==false){
					...
					}else if(this.addInfo.bottle_cover==false){
					...
					if(this.partsAll.win_bottle.length>0){
						if(data.win_bottle[0].isDefaults == 1){
							...
							if(data.win_bottle[0].win_cover&&this.addInfo.bottle_cover==false){
								this.partsAll.win_cover = [];
			...
			getcheckGoods(num){
				...
				if(this.partsAll.bag){
					for (let item of this.partsAll.bag) {
						if (item.checked) {
							let recommendItem = {
								goodsId: this.activeGoodsId,
								productId: item.productId,
