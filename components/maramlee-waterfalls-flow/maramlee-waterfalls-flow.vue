<template>
	<view class="waterfalls-box" :class="{'listStyle':isBlock}" :style="{ height: height + 'px' }">
		<!--  #ifdef  MP-WEIXIN -->
        ...
        left: allPositionArr[index].left || 0,
		 ...
		 @click="$emit('wapper-lick', item)">
			<image class="waterfalls-list-image" mode="widthFix" :class="{ single }" :style="imageStyle" :src="item[imageSrcKey] || item['imageUrl']"
